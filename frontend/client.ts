 ...
 */
export class Client {
    public readonly liquidity: liquidity.ServiceClient
    ...
    constructor(target: BaseURL, options?: ClientOptions) {
        ...
        this.liquidity = new liquidity.ServiceClient(base)
 ...
 */
import { create as api_liquidity_create_create } from "~backend/liquidity/create";
import { get as api_liquidity_get_get } from "~backend/liquidity/get";
...
export namespace liquidity {
    ...
    export class ServiceClient {
         ...
         */
        public async create(params: RequestType<typeof api_liquidity_create_create>): Promise<ResponseType<typeof api_liquidity_create_create>> {
            // Now make the actual call to the API
            const resp = await this.baseClient.callTypedAPI(`/liquidity-controls`, {method: "POST", body: JSON.stringify(params)})
            return JSON.parse(await resp.text(), dateReviver) as ResponseType<typeof api_liquidity_create_create>
        }

        /**
         * Retrieves liquidity control settings for a token.
         */
        public async get(params: RequestType<typeof api_liquidity_get_get>): Promise<ResponseType<typeof api_liquidity_get_get>> {
            ...
            // Now make the actual call to the API
            const resp = await this.baseClient.callTypedAPI(`/liquidity-controls`, {query, method: "GET", body: undefined})
            return JSON.parse(await resp.text(), dateReviver) as ResponseType<typeof api_liquidity_get_get>
        }
    }
}
...
import { deleteToken as api_token_delete_deleteToken } from "~backend/token/delete";
import { get as api_token_get_get } from "~backend/token/get";
import { list as api_token_list_list } from "~backend/token/list";
import { update as api_token_update_update } from "~backend/token/update";
...
export namespace token {
    ...
    export class ServiceClient {
         ...
         */
        public async deleteToken(params: { id: string }): Promise<void> {
            await this.baseClient.callTypedAPI(`/tokens/${encodeURIComponent(params.id)}`, {method: "DELETE", body: undefined})
        }

        /**
         * Retrieves a single token by ID.
         */
        public async get(params: { id: string }): Promise<ResponseType<typeof api_token_get_get>> {
            // Now make the actual call to the API
         ...
         */
        public async update(params: RequestType<typeof api_token_update_update>): Promise<ResponseType<typeof api_token_update_update>> {
            // Construct the body with only the fields which we want encoded within the body (excluding query string or header fields)
            const body: Record<string, any> = {
                freezeAuthorityRevoked: params.freezeAuthorityRevoked,
                mintAddress:            params.mintAddress,
                mintAuthorityRevoked:   params.mintAuthorityRevoked,
                transactionSignature:   params.transactionSignature,
                updateAuthorityRevoked: params.updateAuthorityRevoked,
            }

            // Now make the actual call to the API
            const resp = await this.baseClient.callTypedAPI(`/tokens/${encodeURIComponent(params.id)}`, {method: "PATCH", body: JSON.stringify(body)})
            return JSON.parse(await resp.text(), dateReviver) as ResponseType<typeof api_token_update_update>
        }
    }
}
